...
#define xendrm_display_mode_from_videomode drm_display_mode_from_videomode
#else
static void xendrm_display_mode_from_videomode(const struct videomode *vm,
...
static int xendrm_connector_get_modes(struct drm_connector *connector)
{
	...
	xendrm_display_mode_from_videomode(&videomode, mode);
...
static struct drm_plane *xendrm_crtc_create_primary(
	struct xendrm_device *xendrm_du, struct xendrm_crtc *du_crtc)
{
...
static int xendrm_crtc_do_page_flip(struct drm_crtc *crtc,
	...
	uint32_t drm_flags)
{
	struct xendrm_crtc *du_crtc = to_xendrm_crtc(crtc);
	struct drm_device *dev = du_crtc->crtc.dev;
	struct xendrm_device *xendrm_du;
	unsigned long flags;
	int ret;

	if (unlikely(xendrm_crtc_page_flip_pending(du_crtc))) {
	...
	uint64_t fb_cookie)
{
	...
	if (atomic_dec_and_test(&du_crtc->pg_flip_senders))
		xendrm_crtc_ntfy_page_flip_completed(du_crtc);
}

void xendrm_crtc_on_page_flip_to(struct xendrm_crtc *du_crtc)
{
	if (xendrm_crtc_page_flip_pending(du_crtc)) {
...
static int xendrm_crtc_set_config(struct drm_mode_set *set)
{
	...
	struct xendrm_crtc *du_crtc = to_xendrm_crtc(crtc);
	struct xendrm_device *xendrm_du = du_crtc->xendrm_du;
	int ret;
...
static void xendrm_crtc_disable(struct drm_crtc *crtc)
{
	...
	if (wait_event_timeout(du_crtc->flip_wait,
			!xendrm_crtc_page_flip_pending(du_crtc),
	...
	struct xendrm_crtc *du_crtc, unsigned int index)
{
	...
	ret = xendrm_crtc_props_init(xendrm_du, du_crtc);
	if (ret < 0)
		return ret;
	primary = xendrm_crtc_create_primary(xendrm_du, du_crtc);
	if (!primary)
		return -ENOMEM;
