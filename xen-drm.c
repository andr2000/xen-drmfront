...
struct xendrm_buf_ops {
	void *(*dumb_create)(struct drm_file *file_priv,
...
#ifdef CONFIG_DRM_GEM_CMA_HELPER
static void *xendrm_cma_dumb_create(struct drm_file *file_priv,
	struct drm_device *dev, struct drm_mode_create_dumb *args)
{
	...
	struct drm_mode_create_dumb *args)
{
	...
	vaddr = xendrm_buf_ops.dumb_create(file_priv, dev, args);
	if (IS_ERR(vaddr)) {
		ret = PTR_ERR(vaddr);
		goto fail;
	}
	ret = xendrm_du->front_funcs->dbuf_create(
			...
			args->height, args->bpp, args->size, vaddr);
	if (ret < 0)
		goto fail_destroy;
	return 0;
...
{
	return xendrm_buf_ops.mmap(file, vma);
}
